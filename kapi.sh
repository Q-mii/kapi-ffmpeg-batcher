...
while [ $option != '' ]
    ...
    if [ $option = '' ]; then
      ...
      case $option in
       ...
       7f) clear; mkdir -p "[FLAC]"; for i in *.{m4v,mkv,mp4,mov,avi,mxf,asf,ts,vob,3gp,3g2,f4v,flv,ogv,ogx,wbm,divx}; do ffmpeg -i "$i" -c:a flac "[FLAC]/${i%.*}.flac"; done; exit 0;;
        # [8] MAKE 320kbps *.MP3s
        8) clear; for i in *; do ffmpeg -i "$i" -c:a libmp3lame 320k "${i%.*}.mp3"; done; exit 0;;
       8f) clear; mkdir -p "[MP3]"; for i in *.{m4v,mkv,mp4,mov,avi,mxf,asf,ts,vob,3gp,3g2,f4v,flv,ogv,ogx,wbm,divx}; do ffmpeg -i "$i" -c:a libmp3lame 320k "[MP3]/${i%.*}.mp3"; done; exit 0;;
        # [9] MAKE ~128kbps *.OPUS
        9) clear; for i in *; do ffmpeg -i "$i" -c:a libopus -b:a 128k "%%~ni.opus"; done; exit 0;;
       9f) clear; mkdir -p "[OPUS]"; for i in *.{m4v,mkv,mp4,mov,avi,mxf,asf,ts,vob,3gp,3g2,f4v,flv,ogv,ogx,wbm,divx}; do ffmpeg -i "$i" -c:a libopus -b:a 128k "[OPUS]/%%~ni.opus"; done; exit 0;;
